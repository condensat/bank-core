...
func TestGetLastBatchInfo(t *testing.T) {
	...
	data, _ := encoding.EncodeData(&model.BatchInfoCryptoData{
		TxID: "",
	})
...
func createBatchInfo(batchID model.BatchID, status model.BatchStatus, dataType encoding.DataType, data model.BatchInfoData) model.BatchInfo {

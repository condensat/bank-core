...
func Test_cryptoAddressColumnNames(t *testing.T) {
	...
	fields := tests.GetSortedTypeFileds(reflect.TypeOf(model.CryptoAddress{}))
...
func TestAddOrUpdateCryptoAddress(t *testing.T) {
	...
	db := tests.Setup(databaseName, CryptoAddressModel())
...
func checkCryptoAddressUpdate(t *testing.T, db database.Context, ref model.CryptoAddress) {
	// fetch from db
	{
	...
	// change PublicAddress
	{
		...
		cpy.PublicAddress = model.String(tests.RandSeq(4))
